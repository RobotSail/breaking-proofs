...
_vfail=0
_f="$PROJECT_PATH/.factory/strategy/research.md"
[ ! -f "$_f" ] && echo "VERIFY FAIL: researcher: .factory/strategy/research.md missing" && _vfail=1
[ -f "$_f" ] && [ ! -s "$_f" ] && echo "VERIFY FAIL: researcher: .factory/strategy/research.md is empty" && _vfail=1
...
_vfail=0
_f="$PROJECT_PATH/.factory/evolve/candidate.py"
[ ! -f "$_f" ] && echo "VERIFY FAIL: builder: .factory/evolve/candidate.py missing" && _vfail=1
[ -f "$_f" ] && [ ! -s "$_f" ] && echo "VERIFY FAIL: builder: .factory/evolve/candidate.py is empty" && _vfail=1
...
_vfail=0
_f="$PROJECT_PATH/.factory/experiments/$EXP_ID/eval_after.json"
[ ! -f "$_f" ] && echo "VERIFY FAIL: health_checker: .factory/experiments/$EXP_ID/eval_after.json missing" && _vfail=1
[ -f "$_f" ] && [ ! -s "$_f" ] && echo "VERIFY FAIL: health_checker: .factory/experiments/$EXP_ID/eval_after.json is empty" && _vfail=1
...
```bash
# Artifact verification: archivist_final
_vfail=0
_f="$PROJECT_PATH/.factory/archive/evolve-summary.md"
[ ! -f "$_f" ] && echo "VERIFY FAIL: archivist_final: .factory/archive/evolve-summary.md missing" && _vfail=1
[ -f "$_f" ] && [ ! -s "$_f" ] && echo "VERIFY FAIL: archivist_final: .factory/archive/evolve-summary.md is empty" && _vfail=1
[ "$_vfail" -ne 0 ] && echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) VERIFY_FAIL node=archivist_final" >> "$PROJECT_PATH/.factory/hooks/hook-log.txt" && exit 1
echo "VERIFY OK: archivist_final artifacts validated"
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) VERIFY_OK node=archivist_final" >> "$PROJECT_PATH/.factory/hooks/hook-log.txt"
```
*(harness verification — DO NOT SKIP)*
